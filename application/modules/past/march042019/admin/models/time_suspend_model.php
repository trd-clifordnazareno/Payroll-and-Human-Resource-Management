...
class Time_Suspend_Model extends MY_Model{
	...
	function getFields($id){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('time_suspend.column'=>$id));
		$query = $this->db->get();
	...
	function getField($id, $x, $i){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('time_suspend.column'=>$id, 'time_suspend.column'=>$x, 'time_suspend.column'=>$i));
		$query = $this->db->get();
	...
	function getaccount($fname, $lname, $packavailed){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('time_suspend.column'=>$fname, 'time_suspend.column'=>$lname, 'time_suspend.column'=>$packavailed));
		$query = $this->db->get();
	...
	{
		$this->db->from("time_suspend_tbl time_suspend");
	}
	...
	{
		$this->db->where(array('time_suspend.enabled' => 1));
	}
	...
	function insert_table($data){
		$this->db->insert("time_suspend_tbl",$data);
	...
	function insert_batch_table($data){
		$this->db->insert_batch("time_suspend_tbl", $data); 
	}
	...
	function update_table($data,$table_col,$key){
		...
		$this->db->update("time_suspend_tbl",$data);
        return $key;
	}
	...
	function update_batch_table($data,$table_col){
		$this->db->update_batch("time_suspend_tbl", $data, $table_col); 
	}
	...
	function delete_table($table_col,$key){
		$this->db->delete("time_suspend_tbl",array($table_col=>$key));
	}
	...
	*/
	function getFields_arr($where,$select,$return='',$return_value=''){
        ...
        function get_time_suspend($id){
            $data = $this->db->query("select * from time_suspend_tbl where emp_id = '$id' "
                    . "and date_time_suspend between '2018-08-10' and '2018-08-15' "
                    . "and approved = 1"
                    . " group by time_from");
            return $data->result();
        }
