<button type="submit" class="btn btn-primary" onclick="printDiv();" style="margin-bottom:3px; margin-top:-40px; margin-left:39px;">Print Payroll</button>
...
foreach($payroll_data_rec as $data_rec){
    foreach($add_on as $add_on_data){
                      if($data_rec->emp_id == $add_on_data->emp_id){ $add_adjustment = $add_on_data->adjustment; $add_commision = $add_on_data->commision;break;}else{}  
                    }
                    $total = 0;
                    foreach($get_deduction as $get_deduction_data){
                        if($get_deduction_data->emp_id == $data_rec->emp_id){
                            $amount_to_deduct = $get_deduction_data->amount_to_deduct;
                            $total_amount = $amount_to_deduct + $total;
                            $total = $total_amount;
                        }
                    }
                    $total_amount_deduction = $total;
   ...
   echo "<tr>";
    ...
    echo "<td>" . $add_adjustment ."</td>";
    echo "<td>" . $add_commision ."</td>";
    echo "<td>" . $total_amount_deduction ."</td>";
    //end new
    if($data_rec->pay_type == "monthly"){
        ...
        if($absent == 0){
            ...
            echo "<td>" . (((((($get_total_salary_with_deductions + $data_rec->employee_total_over_time_amount) + (($data_rec->legal * $data_rec->employee_sal_per_day) + (($data_rec->special * $data_rec->employee_sal_per_day) * 0.30))) - $data_rec->employee_total_tardiness_amount) + ($sick_num + $vacation_num)) + $add_adjustment + $add_commision) - $total_amount_deduction) . "</td>";
        }
    }else{
        ...
        echo "<td>" . $data_rec->total_absent ."</td>";
        echo "<td>" . ($data_rec->employee_gross_salary - $num_leave) ."</td>";
        echo "<td>" . ((($data_rec->employee_net_salary - $num_leave) + $add_adjustment + $add_commision) - $total_amount_deduction) ."</td>";
    }
