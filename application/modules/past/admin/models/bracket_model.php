...
class Bracket_Model extends MY_Model{
	...
	function getFields($id){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('bracket.column'=>$id));
		$query = $this->db->get();
	...
	function getField($id, $x, $i){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('bracket.column'=>$id, 'bracket.column'=>$x, 'bracket.column'=>$i));
		$query = $this->db->get();
	...
	function getaccount($fname, $lname, $packavailed){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('bracket.column'=>$fname, 'bracket.column'=>$lname, 'bracket.column'=>$packavailed));
		$query = $this->db->get();
	...
	{
		$this->db->from("bracket_type_tbl bracket");
	}
	...
	{
		$this->db->where(array('bracket.enabled' => 1));
	}
	...
	function insert_table($data){
		$this->db->insert("bracket_type_tbl",$data);
	...
	function insert_batch_table($data){
		$this->db->insert_batch("bracket_type_tbl", $data); 
	}
	...
	function update_table($data,$table_col,$key){
		...
		$this->db->update("bracket_type_tbl",$data);
        return $key;
	}
	...
	function update_batch_table($data,$table_col){
		$this->db->update_batch("bracket_type_tbl", $data, $table_col); 
	}
	...
	function delete_table($table_col,$key){
		$this->db->delete("bracket_type_tbl",array($table_col=>$key));
	}
