	...
	*/
	function getFields_arr($where,$select,$return='',$return_value=''){
        ...
        function get_all_rec(){
            //$this->db->query("DELETE date1 FROM employee_time_record_tbl date1, employee_time_record_tbl date2 WHERE date1.date_time_record = date2.date_time_record");
            //$data = $this->db->query("select * from computational_time_tbl");
            $data = $this->db->query("select DISTINCT emp_id, emp_name, emp_name, date_time_in, date_time_out, date, over_time_total_hour, under_time_total_hour, "
                    . "total_accomplished_hours, total_hours_to_comply_daily, holiday_pay, holiday_hourly_pay, over_time_premium, day_type, leave_from, leave_to, absent, "
                    . "schedue_time, time_suspend, tardiness_amount_to_deduct, 	total_hours_of_juty_required, 	total_normal_pay, tardiness_time_total, late_in_time_status, "
                    . "late_hour_no_deduction, 	total_pay_for_over_all_hour, late_leave, over_time_amount, salary_per_day_amount, salary_per_hour_amount, total_hours_of_juty_complied	 "
                    . "from computational_time_tbl where enabled = 1 order by date");
            return $data->result();
        }
        ...
        function get_overitme_number(){
            $this->db->select_sum('over_time_amount');
            $query = $this->db->get('computational_time_tbl');
            return $query->result();
        }
