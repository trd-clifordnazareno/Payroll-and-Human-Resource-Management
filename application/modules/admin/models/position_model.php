...
class Position_Model extends MY_Model{
	...
	function getFields($id){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('position.column'=>$id));
		$query = $this->db->get();
	...
	function getField($id, $x, $i){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('position.column'=>$id, 'position.column'=>$x, 'position.column'=>$i));
		$query = $this->db->get();
	...
	function getaccount($fname, $lname, $packavailed){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('position.column'=>$fname, 'position.column'=>$lname, 'position.column'=>$packavailed));
		$query = $this->db->get();
	...
	{
		$this->db->from("position_tbl position");
	}
	...
	{
		$this->db->where(array('position.enabled' => 1));
	}
	...
	function insert_batch_table($data){
		$this->db->insert_batch("position_tbl", $data); 
	}
	...
	function update_batch_table($data,$table_col){
		$this->db->update_batch("position_tbl", $data, $table_col); 
	}
	...
	function delete_table($table_col,$key){
		$this->db->delete("position_tbl",array($table_col=>$key));
	}
	...
	*/
    ...
    function getSearch_position($where = array(), $group_by = array(), $order_by = array(), $result = FALSE, $count = FALSE , $row = FALSE){
		self::_select();
		self::_from();
		self::_join();
		self::_fix_arg();
	...
	function get_all_pos_list(){
            $data = $this->db->query("SELECT * FROM position_tbl where enabled = 1;");
            return $data->result();
        }
