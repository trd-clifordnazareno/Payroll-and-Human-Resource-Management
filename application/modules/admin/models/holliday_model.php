...
class Holliday_Model extends MY_Model{
	...
	function insert_table($data){
		$this->db->insert("holliday_tbl ",$data);
...
WHERE end BETWEEN '$date_start' AND '$date_to' and enabled = 1;");
        ...
        function get_all_hollidays(){
            $data = $this->db->query("select * from employee_time_record_tbl where holiday_type != '' and enabled = 1 group by date_time_record order by date_time_record");
            return $data->result();
        }
		function insert_table_holliday($data){
		$this->db->insert("employee_time_record_tbl",$data);
		return $this->db->insert_id();
	}
	function delete_spicific_date($table_col, $key){
