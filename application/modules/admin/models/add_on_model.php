...
class Add_On_Model extends MY_Model{
	...
	function insert_table($data){
		$this->db->insert("add_on_tbl",$data);
...
WHERE tos BETWEEN '$date_start' AND '$date_to' and enabled = 1;"
		...
		function get_deduction($date_start, $date_to){
            /*$data = $this->db->query("SELECT * FROM deduction_item
WHERE end BETWEEN '$date_start' AND '$date_to' and enabled = 1;");
            return $data->result();*/
			$data = $this->db->query("SELECT * FROM deduction_item
WHERE start <= '$date_start' AND end >= '$date_to' and enabled = 1;");
            return $data->result();
        }
		function get_add_on_rec(){
            $data = $this->db->query("select * from add_on_tbl where enabled = 1");
            return $data->result();
        }
        function get_overitme_number($emp_id){
            $data = $this->db->query("select sum(over_time_total_hour) as ot from computational_time_tbl where emp_id = '$emp_id' and over_time_total_hour >= '1.45' and over_time_total_hour != '1.5'");
            return $data->result();
        }
        function get_overitme_amount($emp_id){
            $data = $this->db->query("select sum(over_time_amount) as otm from computational_time_tbl where emp_id = '$emp_id' and over_time_total_hour >= '1.45' and over_time_total_hour != '1.5'");
            return $data->result();
        }
        function get_extended_ot($emp_id, $date_start, $date_to){
            $data = $this->db->query("select * from over_time_authorization_tbl where emp_id = '$emp_id' and enabled = 1 and date(date_over_time) between '$date_start' and '$date_to'");
            return $data->result();
        }
        function get_date_half_day_leave_excess($from, $to){
            $data = $this->db->query("select * from employee_time_record_tbl where date(date_time_record) between $from and $to and enabled = 1");
            return $data->result();
        }
        function get_date_that_has_half_day_leave_in_computational_table(){
            $data = $this->db->query("select * from computational_time_tbl where enabled = 1");
            return $data->result();
        }
		//new
		function update_zeros_enabled(){
		$this->db->query("update computational_time_tbl set enabled = 1 where enabled = 0");
	}
