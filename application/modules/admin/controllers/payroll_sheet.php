        ...
        function request_payroll_rec(){
            ...
            if($data_emp_daily_record){
                foreach($data_emp_daily_record as $data_emp_daily_record_data){
                    ...
                    $check_data_leave = computational_time_model::get_leave_record($data_emp_daily_record_data->date_time_record, $data_emp_daily_record_data->emp_id);
                    foreach($check_data_leave as $data_rec){
                        $data_date = $data_rec->leave_from;
                        $data_id = $data_rec->emp_id;
                        //computational_time_model::update_leave_specific_date($data_date, $data_id);
                    }
                          ...
                          if($sched_time < $date_time_in && $time_out == $date_time_out){ //late in time in
                           ...
                           //echo $total_tardiness_time_var / 60 . "</br>";
                             ...
                             if($over_time_tot_hour >= "1.45" && $over_time_tot_hour <= "2.0"){
                                    ...
                                    $over_time_tot_hour = $over_time_tot_hour;
                                    //$over_time_amt = $over_time_amt;
                                }else if($over_time_tot_hour >= "1.0" && $over_time_tot_hour <= "1.44"){
                                    $over_time_tot_hour = $over_time_tot_hour;
                                    //$over_time_amt = $salary_per_hour_amt;
                                }else{
                             ...
                             for ($x=strtotime($date_time);$x<=strtotime($date_time);$x+=86400){
                                ...
                                if($d_date == "Saturday"){
                                    if($time_in_date < $sched_time){
                                         ...
                                         $data_to_ins_computational_model['total_accomplished_hours'] = $total_hours_for_the_day + round(($full_tardiness1 / 60));//$total_hours_for_the_day;
                                         $data_to_ins_computational_model['total_hours_to_comply_daily'] = $tot_hours_to_comply;
                                         $data_to_ins_computational_model['tardiness_amount_to_deduct'] = 0;//$tardiness_amt;//$tardiness_amt;
                                         $data_to_ins_computational_model['tardiness_time_total'] = 0;//$full_tardiness1;//$full_tardiness1;
                                         
                                         $get_over_time_authorization = employee_model::getFieldsforOverTimeAutorization("$emp_id_ins", $date);
										if($get_over_time_authorization){
											$over_time_add_amount_excess = 0;
                                        }else{$over_time_add_amount_excess = $over_time_amt;}
                                        
                                        
                                         $data_to_ins_computational_model['total_pay_for_over_all_hour'] = $salary_per_day_amt - $over_time_add_amount_excess;//($tot_pay_day);//round($tot_pay_day);
                             ...
                             computational_time_model::insert_table($data_to_ins_computational_model);
                                        ...
                                        $sched_out_exist = $sched_out;//$in_out;//time in
                                        $in_out_exist = "08:46:00";// time out fix

                                        $diff_data_i_out_exist = strtotime($sched_out_exist) - strtotime($in_out_exist);
                                        ...
                                        $compute_late = $late_exist;//$late - $late_exist;
                                        ...
                                        $data_to_ins_computational_model['emp_id'] = $emp_id_ins;
                                         ...
                                         $data_to_ins_computational_model['tardiness_time_total'] = $late_exist;//$late - $late_exist;//$full_tardiness1;
										...
										if($get_over_time_authorization){
                                         ...
                                         $data_to_ins_computational_model['total_pay_for_over_all_hour'] = $total_salary_per_day_amount - $over_time_add_amount_excess;//$salary_per_day_amt;//($tot_pay_day);//round($tot_pay_day);
                                         $data_to_ins_computational_model['total_hours_of_juty_required'] = $tot_hours_of_juty_required;
                                         $data_to_ins_computational_model['total_normal_pay'] = $total_salary_per_day_amount;//$salary_per_day_amt;//$hour_pay;
                                         $data_to_ins_computational_model['over_time_amount'] = ($over_time_amt);//round($over_time_amt);
                                         $data_to_ins_computational_model['salary_per_day_amount'] = $salary_per_day_amt;
                                         $data_to_ins_computational_model['salary_per_hour_amount'] = $salary_per_hour_amt;
                                         $data_to_ins_computational_model['enabled'] = 1;
                             ...
                             computational_time_model::insert_table($data_to_ins_computational_model);
										...
										if($get_over_time_authorization){
                                         ...
                                         $data_to_ins_computational_model['total_pay_for_over_all_hour'] = ($tot_pay_day - $over_time_add_amount_excess);//round($tot_pay_day);
                                         $data_to_ins_computational_model['total_hours_of_juty_required'] = $tot_hours_of_juty_required;
                 ...
                 /// varibales to be injected in employee time record model
                            ...
                            if($leave_data){
                                foreach($leave_data as $leave_data_data){
								   ...
								   $half_day = $leave_data_data->half_day;
								   $check_half_day = $leave_data_data->half_day;
                                                    ...
                                                    echo "no pay";
                                                        ...
                                                        for ($x=strtotime($leave_from_data);$x<=strtotime($leave_to_data);$x+=86400){
                                                            if($leave_data_data->use == 0){
																if($half_day == 1){
																	$cal_leaves = $checked->emp_leave_available - "0.5";
                                                                $xxx = array(
                                                                ...
                                                                if($checked->juty_day_to == "Friday"){
                                                                    if($d_date != "Saturday" && $d_date != "Sunday"){
                                                                        $data_['date'] = date('Y-m-d',$x);
																		 ...
																		 if($check_half_day == 1){
																	 $data_['total_pay_for_over_all_hour'] = $checked->daily_salary / 2;
																	 $data_['salary_per_day_amount'] = $checked->daily_salary / 2;
																 }else{
                                                        ...
                                                        for ($x=strtotime($leave_from_data);$x<=strtotime($leave_to_data);$x+=86400){
                                                            if($use_var == 0){
																if($half_day == 1){
																	$cal_leaves = $checked->emp_sick_leave_available - "0.5";
                                                                $xxx = array(
                            ...
                            /*$get_leave_list = computational_time_model::get_leave_list();
                                    if($get_leave_list){
                                        
                                        foreach($get_leave_list as $get_leave_list_data){
                                            
                                            $date_data = $get_leave_list_data->leave_from;
                                            $empid_data = $get_leave_list_data->emp_id;
                                            
                                            $get_specific_work_in_computational_table = computational_time_model::get_specific_work_in_computational_table($date_data, $empid_data);
                                            
                                            if($get_specific_work_in_computational_table){
                                                $num_count = 0;
                                                foreach($get_specific_work_in_computational_table as $get_specific_work_in_computational_table_data){
                                                    $check_vacation = $get_specific_work_in_computational_table_data->total_hours_of_juty_required;
                                                    if($check_vacation == "vacation leave" || $check_vacation == "sick leave"){
                                                        computational_time_model::update_computational_fileleave($date_data, $empid_data);
                                                        //echo $get_specific_work_in_computational_table_data->emp_id;
                                                    }else if($check_vacation != "vacation leave" || $check_vacation != "sick leave"){
                                                        computational_time_model::update_computational_jutyleave($date_data, $empid_data);
                                                    }
                                                }
                                                //leave_model::insert_new_work_juty_time($date_data, $empid_data);
                                            }
                                        }
                                    }*/
                                ...
                                $check_emp_id = employee_model::getSearch(array('employee.emp_code ='=>$emp_id_data),"",array("employee.emp_id"=>"ASC"),true);
                                    foreach($check_emp_id as $employee_data){
                                        ...
                                        $get_hol_legal_count = employee_time_record_model::get_hol_count($date_start, $date_to, "legal", $employee_id);
                                         ...
                                         //echo "$get_all_records_from_computational_table_data->d" . "</br>";
                                         ...
                                         //walay gamit
                                         $get_overitme_number = add_on_model::get_overitme_number($emp_id_data);
                                         foreach($get_overitme_number as $get_overitme_number_data){
                                             $employee_total_over_time = $get_overitme_number_data->ot;break;
                                             //$employee_total_over_time_amount = $get_overitme_number_data->otm;
                                         }
                                         $get_overitme_amount = add_on_model::get_overitme_amount($emp_id_data);
                                         foreach($get_overitme_amount as $get_overitme_amount_data){
                                             $employee_total_over_time_amount = $get_overitme_amount_data->otm;break;
                                         }
                                         //walay gamit
                                         ...
                                         $get_extended_ot = add_on_model::get_extended_ot($emp_id_data, $date_start, $date_to);
                                         if($get_extended_ot){
                                             $num_of_hours_over_time_extend = 0;
                                             $num_of_over_time_amount_extend = 0;
                                         foreach($get_extended_ot as $get_extended_ot_data){
                                             $date_over_time_extend = $get_extended_ot_data->time_over_from;
                                             $time_over_from_extend = $get_extended_ot_data->over_time_to;
                                                
                                                $from_extend       = $date_over_time_extend;
                                                $to_extend         = $time_over_from_extend;

                                                $total_extend      = strtotime($to_extend) - strtotime($from_extend);
                                                $hours_extend      = floor($total_extend / 60 / 60);
                                                $minutes_extend    = round(($total_extend - ($hours_extend * 60 * 60)) / 60);
                                                
                                                $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                
                                                
                                                if($tot_extend >= "1.45" && $tot_extend <= "2.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "2.45" && $tot_extend <= "3.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "3.45" && $tot_extend <= "4.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "4.45" && $tot_extend <= "5.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "5.45" && $tot_extend <= "6.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "6.45" && $tot_extend <= "7.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "7.45" && $tot_extend <= "8.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "8.45" && $tot_extend <= "9.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "9.45" && $tot_extend <= "10.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }else if($tot_extend >= "10.45" && $tot_extend <= "11.0"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
                                                }/*else if($tot_extend >= "1.0" && $tot_extend <= "1.44"){
                                                    $tot_extend = 0;
                                                }*/
												else if($tot_extend >= "1.0" && $tot_extend <= "1.44"){
                                                    $tot_extend = $hours_extend.'.'.$minutes_extend;
												}
                                                else if($$tot_extend < "1.0"){
                                                    $tot_extend = 0;
                                                }else{
                                                    $tot_extend = 0;
                                                }
                                                ...
                                                $num_of_hours_over_time_extend = $num_of_hours_over_time_extend + $tot_extend;
                                                
                                                $over_time_amount_extend = ($employee_sal_per_hour * 1.25) * $tot_extend;
                                                $num_of_over_time_amount_extend = $num_of_over_time_amount_extend + $over_time_amount_extend;
                                                //echo $num_of_hours_over_time_extend;
                                         }
                                         $employee_total_over_time = $num_of_hours_over_time_extend;
                                         $employee_total_over_time_amount = $num_of_over_time_amount_extend;
                                         //echo "<pre>";print_r($employee_total_over_time);echo "<?pre>";
                                                    if($employee_total_over_time >= "1.45" && $employee_total_over_time <= "2.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "2.45" && $employee_total_over_time <= "3.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "3.45" && $employee_total_over_time <= "4.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "4.45" && $employee_total_over_time <= "5.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "5.45" && $employee_total_over_time <= "6.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "6.45" && $employee_total_over_time <= "7.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "7.45" && $employee_total_over_time <= "8.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "8.45" && $employee_total_over_time <= "9.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "9.45" && $employee_total_over_time <= "10.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }else if($employee_total_over_time >= "10.45" && $employee_total_over_time <= "11.0"){
                                               $employee_total_over_time = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;
                                           }/*else if($employee_total_over_time >= "1.0" && $employee_total_over_time <= "1.44"){
                                               $employee_total_over_time = $employee_total_over_time;//$over_time_tot_hour = $employee_total_over_time;
                                               $employee_total_over_time_amount = 0;//$over_time_amt = 0;
                                           }*/
										   else if($employee_total_over_time >= "1.0" && $employee_total_over_time <= "1.44"){
                                               $employee_total_over_time = $employee_total_over_time;//$over_time_tot_hour = $employee_total_over_time;
                                               $employee_total_over_time_amount = $employee_total_over_time_amount;//$over_time_amt = 0;
                                           }else{
                                               $employee_total_over_time = "0";
                                               $employee_total_over_time_amount = "0";
                                           }
                                                    ...
                                                    $data_payroll['employee_total_over_time'] = $employee_total_over_time;//$over_time_tot_hour;
                                                    $data_payroll['employee_total_over_time_amount'] = $employee_total_over_time_amount;//"$over_time_amt";
                            ...
                            for ($x=strtotime($date_start);$x<=strtotime($date_to);$x+=86400){
                                    ...
                                    $get_all_emp_list = employee_model::get_emp_records_list();
                                    foreach($get_all_emp_list as $get_all_emp_list_data){
                                        $employee_id_data_for_leave_excess = $get_all_emp_list_data->emp_code;//echo $employee_id_data_for_leave_excess . "</br>";
                                        /*if($employee_id_data_for_leave_excess == "10035"){
                                            $datasss = computational_time_model::update_exess_half_day_leave($date_start, $date_to, $employee_id_data_for_leave_excess);
                                            foreach($datasss as $datasss_data){
                                                echo $datasss_data->x . "-" .  $datasss_data->date . "-" . $datasss_data->computational_time_id . "</br>";
                                                if($datasss_data->x >= 3){
                                                    $aaa = computational_time_model::get_id($datasss_data->computational_time_id);
                                                    if($aaa){
                                                        $data_exceed_leave = $datasss_data->computational_time_id + 1;
                                                        $data_date_exceed = $datasss_data->date;
                                                        computational_time_model::del_exceed_leave($data_exceed_leave, $data_date_exceed);
                                                    }
                                                }
                                            }
                                        }*/
                                        ...
                                        switch ($employee_id_data_for_leave_excess) {
                                            case "10035":
                                                $datasss = computational_time_model::update_exess_half_day_leave($date_start, $date_to, $employee_id_data_for_leave_excess);
                                                foreach($datasss as $datasss_data){
                                                    //echo $datasss_data->x . "-" .  $datasss_data->date . "-" . $datasss_data->computational_time_id . "</br>";
                                ...
                                ///new update
								add_on_model::update_zeros_enabled();
            ...
            $data_to_display_payroll['deduct_from'] = date("d", strtotime($date_start));
            $data_to_display_payroll['deduct_to'] = date("d", strtotime($date_to)); 
            
            ///new
            $data_to_display_payroll['get_computational_model_data'] = computational_time_model::get_computational_model_data();
            ///nwe
            
            
            
            ///new
            $date_specify_from = strtotime($date_start);
            $date_specify_to = strtotime($date_to);
            $get_date_half_day_leave_excess = add_on_model::get_date_half_day_leave_excess($date_start, $date_to);
            $get_date_that_has_half_day_leave_in_computational_table = add_on_model::get_date_that_has_half_day_leave_in_computational_table();
            $data_to_display_payroll['get_date_half_day_leave_excess'] = $get_date_half_day_leave_excess;
            $data_to_display_payroll['get_date_that_has_half_day_leave_in_computational_table'] = $get_date_that_has_half_day_leave_in_computational_table;
            ///end new
            ///new
            $get_number_of_half_days = leave_model::get_number_of_half_days($date_start, $date_to);
			$data_to_display_payroll['get_number_of_half_days'] = $get_number_of_half_days;
			///end new
            ///new
