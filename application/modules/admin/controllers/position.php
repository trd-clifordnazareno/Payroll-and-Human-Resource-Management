<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Position extends MX_Controller {
        function __construct() {
            parent::__construct();

            $this->load->model('admin/employee_time_record_model');
            $this->load->model('admin/employee_model');
            $this->load->model('admin/leave_model');
            $this->load->model('admin/time_suspend_model');
            $this->load->model('admin/computational_time_model');
            $this->load->model('admin/bracket_for_gov_remettance_model');
            $this->load->model('admin/bracket_model');
            $this->load->model('admin/position_model');
            $this->load->model('admin/dept_model');
            error_reporting(0);
        }
    function index(){
        
        $get_position = position_model::getSearch_position(array(),"",array("position.position_id"=>"ASC"),true);
        $get_dept = dept_model::getSearch_dept(array(),"",array("dept.dept_id"=>"ASC"),true);
        
        $check_log = modules::run("templ/payroll/check_login");
        if($check_log == TRUE){
            //redirect('admin/employee');
            $data['title'] = "Position";
            $data['location'] = "Position";
            
            
            if($this->session->userdata('session_log')){
                $data_sess = $this->session->userdata('session_log');
                    foreach($data_sess as $key => $value){
                        if($key == 'sess_user_type'){
                            $usertype = $value;
                        }
                    }
            }
            $data['user'] = $usertype;
            $data['position_rec'] = $get_position;
            $data['dept_rec'] = $get_dept;
            
            
            echo modules::run("templ/payroll/position_page", $data);
        }else if($check_log == FALSE){
            $data = array(
            'title' => 'Login'
        );
            echo Modules::run("templ/payroll/login_page");
        }
        
    }
	function request(){
            if($this->uri->rsegment(3) == "create_position"){
                self::request_create_position();
            }
        }
    
    function del_position(){
        $dept_id = $this->uri->rsegment(3);
        $data['enabled'] = 0;
        position_model::update_table($data, "position_id", $dept_id);
        
        $this->load->library('user_agent');
        redirect($this->agent->referrer());
    }
    function request_create_position(){
        $position_name = $this->input->post('position_name');
        $dept_id = $this->input->post('dept_id');
        
        $this->load->library("form_validation");
        
        $this->form_validation->set_rules('position_name', 'Position Name', 'trim|required');
        $this->form_validation->set_rules('dept_id', 'Department Name', 'trim|required');
        
        if($this->form_validation->run() == false){
               echo "<div class='alert alert-danger alert-dismissible'>
               <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
               <h4><i class='icon fa fa-ban'></i> Empty Fields!</h4>
                                                    
                </div>";
           }else{
            $data['position_name'] = $position_name;
            $data['dept_id'] = $dept_id;
            $data['enabled'] = 1;
            
            position_model::insert_table($data);
            
            echo "<div class='alert alert-success alert-dismissible'>
               <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
               <h4><i class='icon fa fa-ban'></i> Position Successfully Registered!</h4>
                                                    
                </div>";
        }
    }
    
    

}
?>