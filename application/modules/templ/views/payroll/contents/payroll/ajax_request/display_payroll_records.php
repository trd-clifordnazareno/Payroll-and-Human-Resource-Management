...
foreach($payroll_data_rec as $data_rec){
   ...
   echo "<tr>";
    ...
    echo "<td>" . $data_rec->employee_total_hours_to_comply ."</td>";
	 ...
	 $leave_half_day_num = 0;
	 if($get_number_of_half_days == NULL){
		 $data_result = $data_rec->employee_total_tardiness_time;
	 }else{
		 foreach($get_number_of_half_days as $get_number_of_half_days_data){
		 $emp_id_get_number_of_half_days_data = $get_number_of_half_days_data->emp_id;
		 if($emp_id_get_number_of_half_days_data == $data_rec->emp_id){
			 $leave_half_day_num = $leave_half_day_num + 1;
			 
		 }
		 $data_all_half_day = $leave_half_day_num * 240;
		 $data_result = $data_rec->employee_total_tardiness_time - $data_all_half_day;
		 
		}
	 }
	 
	 echo "<td>" . $data_result ."</td>";
    //echo "<td>" . $data_rec->employee_total_tardiness_amount ."</td>";
	$hourly_rate_data = $data_rec->employee_sal_per_hour;
	$min_data = 60;
	$total_late_data = $data_result;
	$computed_late_amount = ($hourly_rate_data / $min_data) * $total_late_data;
	echo "<td>" . $computed_late_amount ."</td>";
    ...
    foreach($emp_list as $emp_list_data){
            ...
            if($emp_code_get == $data_rec->emp_id){
                $get_ewt = $emp_list_data->ewt;
                echo "<td>" . $emp_list_data->ewt ."</td>";
                break;
            }
    }
    ...
    foreach($get_deduction as $get_deduction_data){
        ...
        $deduction_condition = $get_deduction_data->deduction_condition;
        
        if($add_on_emp_id == $data_rec->emp_id){
            if($deduction_condition == "1"){
                $amount_deduction = $get_deduction_data->amount_to_deduct;
                $deduction_amount = $deduction_amount + $amount_deduction;
        }else if($deduction_condition == "2"){
            if($deduct_from == "26" && $deduct_to == "10"){
                 $amount_deduction = $get_deduction_data->amount_to_deduct;
                 $deduction_amount = $deduction_amount + $amount_deduction;
            }
        }else if($deduction_condition == "3"){
            if($deduct_from == "11" && $deduct_to == "25"){
                 $amount_deduction = $get_deduction_data->amount_to_deduct;
                 $deduction_amount = $deduction_amount + $amount_deduction;
            }
        }
    ...
    if($data_rec->pay_type == "monthly"){
        ...
        if($absent == 0){
			...
			echo "<td>" . (((((($get_total_salary_with_deductions + $data_rec->employee_total_over_time_amount) + (($data_rec->legal * $data_rec->employee_sal_per_day) + (($data_rec->special * $data_rec->employee_sal_per_day) * 0.30))) - $data_rec->employee_total_tardiness_amount) + ($sick_num + $vacation_num)) + ($add_on_amount + $add_on_commision_amount) - $deduction_amount) + ($data_rec->employee_sss_amount + $data_rec->employee_philh_amount + $data_rec->employee_pag_i_amount)) . "</td>";
            echo "<td>" . (((((($get_total_salary_with_deductions + $data_rec->employee_total_over_time_amount) + (($data_rec->legal * $data_rec->employee_sal_per_day) + (($data_rec->special * $data_rec->employee_sal_per_day) * 0.30))) - $data_rec->employee_total_tardiness_amount) + ($sick_num + $vacation_num)) + ($add_on_amount + $add_on_commision_amount) - $deduction_amount) - $get_ewt) . "</td>";
        }
    }else{
        foreach($emp_list as $emp_list_data){
        ...
        ///new update codes
    foreach($get_computational_model_data as $get_computational_model_data_data){
        if($data_rec->emp_id == $get_computational_model_data_data->emp_id){
            $sal_per_day_total_amount = $get_computational_model_data_data->total_sal_day;
        }
    }
    ///new update codes
        echo "<td>" . $data_rec->total_absent ." </td>";
        echo "<td>" . (($data_rec->employee_gross_salary + ($add_on_amount + $add_on_commision_amount) - $deduction_amount) + ($data_rec->employee_sss_amount + $data_rec->employee_philh_amount + $data_rec->employee_pag_i_amount)) ."</td>";
        ///new update codes
        echo "<td>" . (((($sal_per_day_total_amount + ($add_on_amount + $add_on_commision_amount) - $deduction_amount) - $get_ewt) - ($data_rec->employee_sss_amount + $data_rec->employee_philh_amount + $data_rec->employee_pag_i_amount)) - $data_rec->employee_total_tardiness_amount) ."</td>";
        ///new update codes
        //echo "<td>" . (($data_rec->employee_net_salary + ($add_on_amount + $add_on_commision_amount) - $deduction_amount) - $get_ewt) ."</td>";
    }
